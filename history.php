...
<body>
    ...
    <div class="container">
        <h1 style="padding-top: 5rem;">Room no
            ...
            echo $room ?></h1>
         <form method="POST">
        <label for="getdate">Select Date </label>
          <input type="date" id="getdate" name="getdate" required>
        ...
        <?php 
           ...
           if($hd !="" && $hd<=$td)  
           {
           echo "Selected Date: ". $hd;
           $hh=23; 
           if($td==$hd)
           $hh=$th;
           $query = "SELECT * FROM roomoccupncy where DATE(timeVal)='" . $hd . "'AND roomno=" . $room . " AND HOUR(timeVal) <='" . $hh . "'";
           ...
           $chart_data .= "{ time: '" . $row["timeVal"] . "', occupancy: " . $row["strength"] . "}, ";
            ...
            if ($chart_data == '') {
                   $temp = "INSERT into roomoccupncy (timeVal, roomno, strength) VALUES";
                    $sql = "";
                  for ($i = 0; $i <= 8; $i++) {
                   $temp = $temp . "('" . $hd . " 0" . $i . ":00:00', " . $room . ", 0), ";
               }
                   $temp = $temp . "('" . $hd . " 0" . $i . ":00:00', " . $room . ", 5), ";
               for ($i = 10; $i <= 20; $i++) {
                  $temp = $temp . "('" . $hd . " " . $i . ":00:00', " . $room . ", " . rand(20, 45) . "), ";
                 }
                  for ($i = 21; $i <= 23; $i++) {
                 $temp = $temp . "('" . $hd . " " . $i . ":00:00', " . $room . ", 0), ";
                   }
